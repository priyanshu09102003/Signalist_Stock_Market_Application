...
const SearchItem = ({ coin, onSelect, isActiveName }: SearchItemProps) => {
  ...
  return (
    <CommandItem
      ...
      className='search-item_two'
    >
      <div className='coin-info'>
        <Image src={coin.thumb} alt={coin.name} width={40} height={40} className='rounded-full' />
...
export const SearchModal = ({
  ...
  const handleSelect = (coinId: string) => {
    ...
    setSearchQuery('');
    setDebouncedQuery('');
    router.push(`/crypto/coins/${coinId}`);
  };
  ...
  return (
    <div id='search-modal'>
      <Button variant='ghost' onClick={() => setOpen(true)} className='trigger border-2'>
        <SearchIcon size={18} />
        Search
        <kbd className='kbd !opacity-100'>
      ...
      >
        <div className='cmd-input'>
          <CommandInput
            placeholder='Search for a token by name or symbol... (Ctrl+C)'
            value={searchQuery}
